...
- [ ] **Resend webhook not yet registered** — `/api/webhooks/resend` is live but Resend needs to be told about it. After deploy: Resend dashboard → Webhooks → add `https://app.next11ven.com/api/webhooks/resend`, copy the signing secret into `RESEND_WEBHOOK_SECRET` env var on Vercel, and enable: `email.sent`, `email.delivered`, `email.opened`, `email.clicked`, `email.bounced`, `email.complained`. Open/click tracking must also be enabled per-domain in Resend settings.
