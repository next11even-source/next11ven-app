...
| Email | Resend | Player onboarding D0 — welcome + profile CTA (step 10) | Newly approved players | **Immediate — fires in the approval request itself**, not via the morning cron. No opt-out suppression (welcome email). `logTouch(player_onboarding_d0)` + `drip_jobs.sent = true` set in-request. If send fails: `sent` stays `false`, cron retries next morning. | **Phase 2 — when `native_onboarding = true`** | `sendPlayerOnboardingD0Email` called directly from `app/api/admin/review/route.ts` on insert. `app/api/cron/drip-reminders` (step 10) is the retry path only. | 7 Sep 2026 |
...
| Email | Resend | Coach onboarding D0 — welcome + first opportunity CTA (step 14) | Newly approved coaches | **Immediate — fires in the approval request itself**, not via the morning cron. No opt-out suppression (welcome email). `logTouch(coach_onboarding_d0)` + `drip_jobs.sent = true` set in-request. If send fails: `sent` stays `false`, cron retries next morning. | **Phase 2 — when `native_onboarding = true`** | `sendCoachOnboardingD0Email` called directly from `app/api/admin/review/route.ts` on insert. Stat: live `activePlayerCount` queried at send time. `app/api/cron/drip-reminders` (step 14) is the retry path only. | 7 Sep 2026 |
...
| SMS | Twilio | Account approved | New player/coach on admin approval | One-off per approval. `sms_opt_in` checked; no `last_sms_at` cap (single send per lifecycle event). | Live | `app/api/admin/review/route.ts` — fires inline (fire-and-forget) on `isApproving = true`. No `logTouch` call. | 7 Sep 2026 |
| Auth email | Supabase Auth | Password reset / magic link | Any user requesting reset; migrated users via `/claim` | Conditional | Live — **unbranded** (Supabase stock template, no custom Resend equivalent). **Structurally invisible to email analytics** — Supabase fires this directly; Resend receives it with no flow tag and no mechanism to attach one. Shows as "Auth system (password reset / magic link)" in the analytics dashboard. Moving it to a custom Resend template via `lib/email.ts` would fix both gaps. | `app/page.tsx` (`supabase.auth.resetPasswordForEmail`); `/claim` + `/set-password` | 28 Aug 2026 |
...
| Channel | System | Trigger | Audience | Frequency / cap | Status | Code location | Last verified |
|---|---|---|---|---|---|---|---|
| Email | Resend | Coach activation D7 — approved coach, never posted AND never messaged, 7+ days since join | Approved coaches with zero opportunities ever-posted AND zero messages ever-sent | Weekly (Thu 09:00 UTC). Fires once per coach. `canSendTouch(48h)` cross-flow guard + `logTouch(coach_activation_d7)` after send. Scheduled Thu (not Wed) so the 49h gap from Tue recommendations always clears the 48h check. | **Live** — 9 Sep 2026 (rescheduled Wed→Thu to fix recommendations clash) | `app/api/cron/coach-activation`, `sendCoachActivationD7Email`. Angle: region player count (actively-looking players in coach's city; platform total if city unset or <3 matches). Fresh DB check at send time (opp + message). `?dryRun=1` + `?to=<email>` | 9 Sep 2026 |
| Email | Resend | Coach activation D21 — still inactive, 14+ days after D7 | Same coaches, after D7 sent, ≥14 days later | Weekly (Thu 09:00 UTC). Fires once per coach. `canSendTouch(48h)` + `logTouch(coach_activation_d21)` | **Live** — 9 Sep 2026 (rescheduled Wed→Thu) | Same cron, `sendCoachActivationD21Email`. Angle: social proof (coaches who posted this week — different lever from D7). After D21, coach drops out of dedicated nudging | 9 Sep 2026 |
...
| Email | Resend | Drip Day 7 — final reminder | Free players, triggering message still unread | 7 days after Day 0. `logTouch(drip_day7)` | Live — **SMS removed 7 Sep 2026** (low volume; Day 0 real-time alert covers the same player) | `sendDripDay7Email`, `app/api/cron/drip-reminders`. Sequence aborts early if player upgrades, opts out, or reads the message | 7 Sep 2026 |
| Email + SMS | Resend + Twilio | Drip Day 0 — coach messaged a free player | Free players (new drip sequence only — fires once, when coach messages a free player with no active drip) | Triggered inline, immediate. `logTouch(drip_day0)` for email. SMS: upgrade-copy nudge, `sms_opt_in` + 1/day `last_sms_at` cap. | Live | `sendDripDay0Email` + SMS inline, fired from `app/api/messages/send/route.ts` (`isDripEligible && pendingDrip === 0` branch); also inserts Day 3 + Day 7 rows into `drip_jobs` | 7 Sep 2026 |
...
| Email | Resend | Post-match "log your game" nudge | Players with active club stint whose likely match day is today, haven't logged | Daily 18:00 UTC. Respects `email_marketing_opt_out` | Live — **SMS removed 7 Sep 2026** (flywheel nudge; email sufficient for low-urgency prompt) | `sendLogNudgeEmail`, `app/api/cron/log-nudge`, `lib/matchDay.ts`. Free — no upsell | 7 Sep 2026 |
...
| Email | Resend | Player Pro welcome (step 20) | Player on first premium activation | **Immediate — fires in the Stripe webhook handler itself**, not via the morning cron. No opt-out suppression (transactional confirmation). `logTouch(player_pro_welcome)` + `drip_jobs.sent = true` set in-request. If send fails: `sent` stays `false`, cron retries next morning. | **Phase 2 — when `native_onboarding = true`** | `sendPlayerProWelcomeEmail` called directly from `app/api/stripe/webhook/route.ts` (`handleSubscriptionChange`) on first activation. `app/api/cron/drip-reminders` (step 20) is the retry path only. | 7 Sep 2026 |
| Email | Resend | Coach Pro welcome (step 21) | Coach on first premium activation | **Immediate — fires in the Stripe webhook handler itself**, not via the morning cron. No opt-out suppression (transactional confirmation). `logTouch(coach_pro_welcome)` + `drip_jobs.sent = true` set in-request. If send fails: `sent` stays `false`, cron retries next morning. | **Phase 2 — when `native_onboarding = true`** | `sendCoachProWelcomeEmail` called directly from same webhook handler. `app/api/cron/drip-reminders` (step 21) is the retry path only. | 7 Sep 2026 |
...
- [x] ~~**`native_onboarding` flag not yet enabled`**~~ — flipped to `true` 7 Sep 2026. Phase 2 live.
- [x] ~~No migration for `drip_jobs_onboarding_unique` partial index~~ — created in migration `20260906000002_email_safety.sql`. Plain `.insert()` used (not upsert); `23505` unique_violation treated as success (re-approval idempotency guard).
- [x] ~~No Coach Pro upgrade confirmation email~~ — closed by step 21 (`sendCoachProWelcomeEmail`). Live — `native_onboarding = true` in production as of 7 Sep 2026.
- [x] ~~`native_onboarding` flag not yet enabled~~ — flipped to `true` 7 Sep 2026. Phase 2 is live. MailerLite sequences are off; native drip_jobs sequences and immediate D0/Pro welcome sends are active.
- [x] ~~Fan onboarding~~ — dropped. Fan signup closed 12 Aug 2026.
- [x] ~~Full SMS audit~~ — complete. 6 send sites confirmed (7 Sep 2026). SMS removed from Drip Day 7 and post-match log-nudge (7 Sep 2026). 4 active SMS flows remain: new message received (+ Drip Day 0 inline), admin approval, coach application-nudge, payment failed. **Future candidate: gate messages/send SMS to first-message-in-conversation only** — proposal in findings, not yet shipped; would remove ~50–70% of messages/send SMS volume while preserving new-contact real-time alert.
