-- Month-by-Month table needs actual cash collected per month, not subscription
-- price × count — some players/coaches are on £0 promo codes, so counting
-- premium subs overstates revenue. stripe.invoices.amount_paid is what actually
-- hit the account (a £0 comped invoice contributes £0 automatically). Mirrors
-- the same source analytics_revenue_stats() already uses for `mrr_trend`.
