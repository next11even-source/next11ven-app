...
function CoachProfileCompletionBar({ profile }: { profile: CoachCompletionProfile }) {
  const { pct, next } = calcCoachCompletion(profile)
  ...
  return (
    <Link href="/dashboard/profile" className="block h-full" style={{ textDecoration: 'none' }}>
      <div className="rounded-2xl px-4 py-3 h-full flex flex-col justify-center gap-2"
        style={{ backgroundColor: '#13172a', border: '1px solid #1e2235' }}>
        <div className="flex items-center gap-3">
          {/* Names the highest-value gap — see the ranking note in profileCompletion.ts */}
            ...
            <Link key={p.id} href={`/dashboard/player/players/${p.id}`} prefetch={false}
              ...
              <div className="relative" style={{ height: 150, backgroundColor: '#1a1f3a' }}>
                {p.avatar_url ? (
                  ...
                  <Image src={p.avatar_url} alt="" fill sizes="175px" loading="eager" className="object-cover object-center" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center"
                    style={{ background: 'linear-gradient(160deg, #13172a 0%, #0d1020 100%)' }}>
                    <Icon icon={User} size={48} label={true} style={{ color: '#1e2235' }} />
