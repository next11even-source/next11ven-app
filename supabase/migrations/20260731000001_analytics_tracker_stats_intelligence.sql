-- Extend analytics_tracker_stats() with the "is this data actually being
-- used for matching/intelligence" numbers: the coach-searchable pool
-- (mirrors the consent gate in /api/coach/performance-search — actively_looking
-- AND performance_stats_public), the opt-in rate on the public-by-default
-- switch, and career_stats depth (pre-platform history captured via backfill
-- + the "place your season" nudge). Additive only — existing fields unchanged.
