-- real_revenue_pence came back £0 for every month. Diagnostic
-- (analytics_tmp_invoice_diag, dropped below) found why: stripe.invoices.subscription
-- is null on every synced row (0 of 475 paid invoices have it set) even though the
-- invoices themselves are real subscription charges — the Stripe FDW simply doesn't
-- populate that column. The `subscription is not null` filter, copied from the
-- pre-existing (and it turns out equally broken/unused) `mrr_trend` query, was
-- silently zeroing out every month. Dropping that filter — status = 'paid' is
-- the only signal this FDW actually gives us, and it's what every other "actual
-- paying" check in this file (analytics_revenue_stats' `paying` CTE) already relies on.

drop function if exists analytics_tmp_invoice_diag();
...
begin
  ...
  select jsonb_build_object(
    ...
    'monthly_table', coalesce((
      ...
      real_revenue as (
        -- Actual cash collected, not subscription price × headcount. A £0 promo
        -- invoice contributes £0 here even though the subscriber counts as "premium".
        select
