...
--  1. Deduplicate MRR and active_subs by customer — the Stripe sync creates new rows
--     on each renewal rather than updating, so active_subs was overcounting ~12 customers.
--     Fix: DISTINCT ON (customer) ordered by start_date DESC keeps only the most recent
--     active sub per customer. Also filters player-only (coach premium not in focus).
--  2. Add ever_signed_in + total_approved for migration progress % in the Telegram report.
