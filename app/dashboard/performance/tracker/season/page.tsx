...
function SeasonWrapInner() {
  ...
  async function handleShare() {
    ...
    const text = `${seasonLabel(season)} so far on NEXT11VEN: ${headline}${summary.avgRating != null ? ` · avg rating ${summary.avgRating}` : ''}${summary.motmCount > 0 ? ` · ${summary.motmCount}x Man of the Match` : ''}`
    if (navigator.share) {
              ...
              <div className="px-5 pt-4 pb-5 space-y-2">
                {summary.motmCount > 0 && (
                  <div className="flex items-center gap-2.5">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="#f59e0b" stroke="#f59e0b" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0">
                      <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" />
                    </svg>
                    <p className="text-sm" style={{ color: '#e8dece' }}>
                      <span className="font-bold">{summary.motmCount}x Man of the Match</span>
                      <span style={{ color: '#8892aa' }}> this season</span>
                    </p>
                  </div>
                )}
                {best && (
