-- Add `signed` and surface `accepted` in monthly_table rows.
--
-- `accepted` was already computed by the accepted_events CTE and output by
-- analytics_platform_stats() but was missing from the MonthRow TypeScript type
-- (caught in the Phase 2 analytics redesign, 15 Sep 2026). No SQL change
-- needed for accepted — only the frontend type was updated.
--
-- `signed` is new: counts status_change_log rows where new_status = 'signed'
-- per calendar month. History exists from 16 Aug 2026 (trigger
-- trg_log_status_change was deployed then); earlier months return 0.
...
begin
  ...
  select jsonb_build_object(
    ...
    'monthly_table', coalesce((
      ...
      signed_events as (
        -- trg_log_status_change deployed 16 Aug 2026; earlier months return 0
        select date_trunc('month', changed_at)::timestamptz as month, count(*)::int as n
        from status_change_log
        where new_status = 'signed'
          and changed_at >= v_earliest_month
        group by 1
      )
      select jsonb_agg(
        jsonb_build_object(
      ...
      left join signed_events  sgn on sgn.month = m.month
    ), '[]'::jsonb),
