...
async function fetchPosts(pageNum: number, viewerId: string): Promise<PostWithAuthor[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      author:profiles!author_id(id, full_name, avatar_url, role, position, city, is_agent),
      post_likes(user_id),
      post_comments(id)
    `)
    .eq('is_deleted', false)
  ...
  return (data as any[]).map(p => ({
    ...
    author: p.author ?? { id: p.author_id, full_name: null, avatar_url: null, role: null, position: null, city: null, is_agent: null },
