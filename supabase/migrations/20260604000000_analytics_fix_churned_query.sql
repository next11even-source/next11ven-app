-- Fix churned count in month-by-month table.
--
-- The old query read stripe.subscriptions using cancel_at / canceled_at (Unix bigints).
-- For subscriptions cancelled at period end, Stripe leaves both fields as 0, so the
-- coalesce/nullif logic returned NULL and the filter dropped every row → churned showed
-- zero for most months.
--
-- Fix: read public.subscriptions (written by our webhook) and use current_period_end
-- (timestamptz) as the churn date, which is always set and is the correct end-of-access date.
...
begin
  select jsonb_build_object(
    ...
    'monthly_table', coalesce((
      ...
      -- Read from public.subscriptions (webhook-written) so current_period_end is always
      -- populated. The old stripe.subscriptions query used cancel_at / canceled_at which
      -- are 0 for period-end cancellations, causing all churned rows to be filtered out.
      churned as (
        select
          date_trunc('month', current_period_end)::timestamptz as month,
          count(*)::int as n
