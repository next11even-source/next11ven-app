...
export function TrackerAdoptionTrends({ trackerStats }: { trackerStats: TrackerStats | null }) {
  ...
  const sparklines = weekly.length >= 2 ? {
    ...
    history: weekly.map(w => ({ label: w.label, value: w.history_rows })),
    contributors: weekly.map(w => ({ label: w.label, value: w.contributors })),
  } : null
  ...
  return (
    <section>
      ...
      {sparklines ? (
        <div className="grid grid-cols-2 gap-2 mb-2">
          {(Object.keys(sparklines) as Array<keyof typeof sparklines>).map(key => {
            ...
            return (
              <div key={key} className="rounded-xl p-3" style={{ backgroundColor: '#13172a', border: '1px solid #1e2235' }}>
                <div className="flex items-baseline justify-between mb-1.5">
                  <span className="text-xs uppercase tracking-wider" style={{ color: '#8892aa' }}>{COL_LABELS[key]}</span>
                  <span className="text-sm font-black tabular-nums" style={{ fontFamily: "'Barlow Condensed', sans-serif", color }}>{latest}</span>
                </div>
      ...
      {/* Raw table — collapsible, same pattern as MonthByMonth */}
      {weekly.length > 0 && (
        <>
          <button
          ...
          >
            <span className="text-xs" style={{ color: '#3a4055' }}>Weekly breakdown</span>
            <span className="text-xs px-2 py-0.5 rounded transition-colors"
              style={{ color: '#8892aa', backgroundColor: tableOpen ? '#1e2235' : 'transparent' }}>
              {tableOpen ? 'Hide' : 'Show raw'}
            </span>
          </button>
