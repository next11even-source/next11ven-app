...
| Email | Resend | Coach gone quiet D1 — posted before, silent ≥28 days | Approved coaches who have posted at least once ever but not in the last 28 days | Weekly (Fri 09:00 UTC). Fires once per coach. `canSendTouch(48h)` cross-flow guard + `logTouch(coach_gone_quiet_d1)` after send | **Live** — 4 Sep 2026 | `app/api/cron/coach-gone-quiet`, `sendCoachGoneQuietD1Email`. Angle: new players since last post (bulk query, in-memory per-coach filter). Fresh per-coach recency check at send time. `?dryRun=1` + `?to=<email>` | 4 Sep 2026 |
| Email | Resend | Coach gone quiet D14 — still silent, 14+ days after D1 | Same coaches, after D1 sent, ≥14 days later | Weekly (Fri 09:00 UTC). Fires once per coach. `canSendTouch(48h)` + `logTouch(coach_gone_quiet_d14)` | **Live** — 4 Sep 2026 | Same cron, `sendCoachGoneQuietD14Email`. Angle: social proof (coaches who posted this week — different lever from D1). After D14, coach drops out of dedicated nudging | 4 Sep 2026 |
