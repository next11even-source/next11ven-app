-- Fix MRR/subscriber counts to exclude 100%-comped subscriptions (friends &
-- family free codes) and dedupe by customer (Stripe Sync Engine writes a new
-- subscriptions row per renewal instead of updating in place, which silently
-- overcounts "active" rows).
--
-- Both fixes were already discovered and validated for analytics_weekly_snapshot()
-- on 2026-06-26 (see 20260626000004_weekly_snapshot_customer_link.sql and
-- 20260626000008_weekly_snapshot_mrr_dedup.sql — confirmed "55 paying / 3 comped
-- of 58 active" against real data). This migration ports the same two fixes into
-- analytics_revenue_stats() and analytics_platform_stats(), which the admin
-- analytics reframe (20260802000000) now leans on for the MRR hero numbers.
--
-- "Paying" = the subscription's customer has >= 1 invoice with status='paid' and
-- amount_paid > 0. invoices.subscription is NULL in this sync-engine deployment,
-- so the join is via customer, not subscription id (same reason the June fix used
-- customer). No partial discounts are in use — a sub is either full price or fully
-- comped — so unit_amount is still correct as the £ figure for paying subs.
...
begin
  with active_deduped as (
    -- One row per customer: their most recent active subscription. Fixes the
    -- Stripe Sync Engine inserting a new row per renewal instead of updating.
  ...
  paying as (
    -- Excludes 100%-comped subscriptions — a sub only counts toward revenue if
    -- its customer has at least one invoice that was genuinely paid.
    select ad.*
    from active_deduped ad
    where exists (
      select 1 from stripe.invoices i
      where i.customer = ad.customer and i.status = 'paid' and i.amount_paid > 0
    )
  )
  select jsonb_build_object(
    ...
    -- Active subs that never generated a genuinely paid invoice (comped / friends & family)
    ...
    -- Already sourced from real paid invoices — unaffected by the comped-sub bug.
    'mrr_trend', coalesce((
    ...
    'coach_net_adds_monthly', coalesce((
      ...
      coach_new as (
        ...
        where p.role = 'coach'
          ...
          and exists (select 1 from stripe.invoices i where i.customer = s.customer and i.status = 'paid' and i.amount_paid > 0)
        group by 1
      ),
      coach_churned as (
        select
...
-- ─────────────────────────────────────────────────────────────────────────────
-- analytics_platform_stats: same "paying" filter on every field that touches
-- Stripe MRR (rolling new/churned MRR, monthly_table deltas, legacy pricing,
-- player premium conversions). Non-Stripe fields (mau, contacts, activation,
-- etc.) are unchanged from 20260802000000.
-- ─────────────────────────────────────────────────────────────────────────────
...
begin
  with legacy_deduped as (
    -- Current-state field (like active_subs) — same renewal-row dedup applies.
    select distinct on (s.customer)
      s.customer,
      (s.items->'data'->0->'price'->>'unit_amount')::bigint as unit_amount_pence
    from stripe.subscriptions s
