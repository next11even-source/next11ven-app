-- Add applications-accepted to the weekly snapshot, blended into a single
-- "connections made" total alongside new conversations rather than reported
-- as its own headline line. Deliberately NOT tracking signings here — there's
-- no reliable signal on the site for a player actually getting signed (see
-- 20260816000011's note on applications having no accepted_at/updated_at),
-- so this stops at the one accept event we can trust: the coach's own
-- accept action, sourced from the 'application_decision' notification
-- (accept-only, see app/api/applications/[id]/route.ts) rather than
-- applications.status, which has no decision timestamp to window on.
--
-- new_conversations mirrors analytics_hero_stats()'s connections_started
-- definition (count of conversations rows created in the window) so the
-- weekly Telegram number and the dashboard hero tile agree on what a
-- "connection" is, just over a 7d window instead of MTD.
...
begin
  select jsonb_build_object(
    ...
    -- Deduplicated per customer (DISTINCT ON) to fix renewal-row overcounting.
    -- Player-only: coach premium is not tracked in the weekly report right now.
    -- ══════════════════════════════════════════════════════════════════════

    'mrr_pence', coalesce((
      select sum(unit_amount_pence)
      from (
    ...
    'active_subs', coalesce((
      select count(*)::int
      from (
        select distinct on (s.customer) s.id
        from stripe.subscriptions s
        join profiles p on p.stripe_customer_id = s.customer
        where s.status = 'active'
          and p.role in ('player', 'admin')
    ...
    -- Kept for milestone detection even though coach premium is not displayed
    'coach_subs', coalesce((
    ...
    -- Migration progress: approved users who have signed into the new app at least once
    ...
    -- Accept-only, sourced from the notification (not applications.status,
    -- which has no decision timestamp to window on — see 20260816000011).
    'applications_accepted', coalesce((
      select count(*)::int from notifications
      where type = 'application_decision' and created_at >= v_start and created_at < v_now
    ), 0),
    ...
    -- Mirrors analytics_hero_stats()'s connections_started definition, just
    -- windowed to 7d instead of MTD, so the weekly and dashboard numbers
