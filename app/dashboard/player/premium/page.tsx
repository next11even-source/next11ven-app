...
export default function PremiumPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      if (!user) return
      supabase.from('profiles').select('role, premium').eq('id', user.id).single().then(({ data }) => {
  ...
  const gradientBg = isCoach
    ? 'linear-gradient(135deg, #3d1a0a 0%, #c2410c 60%, #7c2d12 100%)'
    : 'linear-gradient(135deg, #1a2a6c 0%, #2d5fc4 60%, #1e3a8a 100%)'
  ...
  return (
    <div className="min-h-screen pb-24" style={{ backgroundColor: '#0a0a0a' }}>
      ...
      <div className="px-4 space-y-5">
        {/* Hero card */}
        <div className="rounded-2xl overflow-hidden relative" style={{ background: gradientBg, padding: '28px 24px' }}>
        ...
        {premium ? (
          <div className="space-y-3">
            <div className="rounded-2xl p-4 flex items-start gap-3" style={{ backgroundColor: '#0f1a10', border: `1px solid ${accentColor}40` }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={accentColor} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className="flex-shrink-0 mt-0.5">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl text-center font-black text-base uppercase tracking-widest disabled:opacity-60"
            style={{ backgroundColor: accentColor, color: '#fff', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '0.08em' }}
          >
            {checkoutLoading ? (
              <span className="flex items-center gap-2">
