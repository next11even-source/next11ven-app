-- Layer 5 trends, extended: monthly_table only covered the metrics that
-- existed before this session's Layer 1/2 work — no historical trend for
-- opportunities posted, connections started, application response rate, or
-- accepted outcomes. Without this, none of those can be pruning candidates
-- (Layer 5's own stated purpose) since there's nothing to look flat against.
--
-- "Signed" deliberately NOT added here: status_change_log only started
-- accruing on 16 Aug 2026 (see 20260816000001), so every month before that
-- would show a structural zero that looks like "nobody signed" rather than
-- what it actually is — "we weren't tracking it yet". That's exactly the
-- kind of misleading number this whole rebuild exists to avoid. Revisit once
-- there's a few real months of history.
