-- Extend monthly_table from a fixed 6-month rolling window to the earliest
-- real activity in the platform. Legacy Glide conversation/profile data was
-- restored into `profiles` and `conversations` (2025 rows) so trends could be
-- seen over a longer history — but every window in this function was
-- `now() - interval '6 months'`, which structurally excludes anything before
-- ~Feb-Mar 2026 no matter how much older data exists. The legacy rows were
-- sitting in the DB, invisible to every chart on the analytics tab.
--
-- v_earliest_month = the earlier of the first profile or first conversation,
-- floored to month. Applications/opportunities/Stripe revenue genuinely don't
-- exist before the April/May 2026 launch, so those columns read an honest
-- zero for 2025 months — same "structural zero, not a bug" pattern already
-- used for `signed` elsewhere in this function.
...
declare
  v_result jsonb;
  v_earliest_month timestamptz;
begin
  select date_trunc('month', least(
    coalesce((select min(created_at) from profiles where role in ('player', 'coach', 'admin')), now()),
    coalesce((select min(created_at) from conversations), now())
  )) into v_earliest_month;
  ...
  select jsonb_build_object(
    ...
    'monthly_table', coalesce((
      ...
      signups as (
        ...
        from profiles
        where role in ('player', 'coach', 'admin')
          and created_at >= v_earliest_month
        group by 1
      ),
      new_subs as (
        select
        ...
        from stripe.subscriptions s
        join latest_invoice li on li.customer = s.customer
        where to_timestamp(s.start_date) >= v_earliest_month
          and s.items->'data'->0->'price'->>'unit_amount' is not null
          and li.status = 'paid' and li.amount_paid > 0
        group by 1
      ),
      churned as (
        ...
        where (s.cancel_at_period_end = true or s.status = 'canceled')
          ...
          and to_timestamp(coalesce(nullif(s.cancel_at, 0), nullif(s.canceled_at, 0))) >= v_earliest_month
          and s.items->'data'->0->'price'->>'unit_amount' is not null
          and li.status = 'paid' and li.amount_paid > 0
        group by 1
      ),
      msgs as (
        select date_trunc('month', created_at)::timestamptz as month, count(*)::int as n
        from messages
        where created_at >= v_earliest_month
        group by 1
      ),
      apps as (
        select date_trunc('month', created_at)::timestamptz as month, count(*)::int as n
      ...
      real_revenue as (
        ...
        from stripe.invoices i
        where i.status = 'paid'
          and to_timestamp(i.period_start) >= v_earliest_month
        group by 1
      ),
      opps as (
        select date_trunc('month', created_at)::timestamptz as month, count(*)::int as n
      ...
      app_response as (
        -- Response rate AS OF NOW for applications that landed in that month —
        -- older months naturally read higher since they've had longer to
        -- resolve. That's the honest shape of this number, not a bug to fix.
        select
      ...
      left join accepted_events acc on acc.month = m.month
    ), '[]'::jsonb),
