-- Fix analytics_event_feed(): profiles.role is the user_role enum, not text —
-- initcap() only accepts text, so the premium_upgraded CTE's fallback label
-- (used only when full_name is null) errored with "function initcap(user_role)
-- does not exist" on every call. Caught by verifying the RPC directly against
-- production before trusting it, same as every other function in this batch.
