...
async function fetchPosts(pageNum: number, viewerId: string): Promise<PostWithAuthor[]> {
  ...
  const { data: authors } = await supabase
    ...
    .select('id, full_name, avatar_url, role, position, city, is_agent')
    .in('id', authorIds)
  ...
  return (data as any[]).map(p => ({
    ...
    author: authorMap.get(p.author_id) ?? { id: p.author_id, full_name: null, avatar_url: null, role: null, position: null, city: null, is_agent: null },
